...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const lastVisitorsResponse = await api.get('/visitors/last', { params: { limit: 5 } });
        ...
        startCountIncrement(userCount, patientCount, doctorCount,visitorCount);
        setLastPatients(lastPatientsData);
  ...
  const startCountIncrement = (userCount, patientCount, doctorCount,visitorCount) => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentUsers === userCount && currentVisitors === visitorCount && currentPatients === patientCount && currentDoctors === doctorCount) {
        clearInterval(interval);
      }
    }, 10);
  };

  const { users, patients, doctors, visitors } = counts;
